...
class TransferLearningHelper(
    ...
    private fun setupModelPersonalization(): Boolean {
        ...
        return try {
            val modelFile = FileUtil.loadMappedFile(context, "modell.tflite")
    ...
    fun addSample(image: Bitmap, className: String, rotation: Int) {
        synchronized(lock) {
            ...
            Log.e("sdsdsdsds", className)

            Log.e("sdsdsdsds", encoding(classes.getValue(className)).toString())
...
//                while (executor?.isShutdown == false) {
                    ...
                    Log.e("size:",trainingSamples.size .toString())
...
//                }
    ...
    private fun training(
        ...
        val lossss = FloatBuffer.allocate(1)

        outputs[TRAINING_OUTPUT_KEY] = loss
        outputs["test"] = lossss

        val outputssdsds: MutableMap<String, Any> = HashMap()
        val array: Array<FloatArray> = Array(1) { FloatArray(4) }
        val wesss: Array<FloatArray> = Array(62720) { FloatArray(4) }
        val lossdss = FloatBuffer.allocate(4)

        outputs["bs"] = array
        outputs["ws"] = wesss

//        val lossdsssds = FloatBuffer.allocate(4)
//        val tensorBuffer = TensorBuffer.createFixedSize(intArrayOf(1, 4), DataType.FLOAT32)

//        outputs["wgradient_tape/add/Reshape:0"] = tensorBuffer
//        outputs["gradient_tape/MatMul/MatMul"] = lossdss

        Log.e("start train","sdsds");
      interpreter?.runSignature(inputs, outputs, TRAINING_KEY)
        Log.e("start train","DONE");
//        val outputss: MutableMap<String, Any> = HashMap()

//
        val inputssdsd: MutableMap<String, Any> = HashMap()

        val outputsGGG: MutableMap<String, Any> = HashMap()
//        outputsGGG["checkpoint_path"] = outputString

     val sdsdsdsds=   interpreter?.getSignatureInputs("save")
        var dsdsputput = interpreter?.getSignatureOutputs("save")
      ...
      val path =context.filesDir.absolutePath
        ...
        val sdsdsdsdsdsds=   interpreter?.getSignatureInputs("train")
...
//        val inputStrings = arrayOf("1.0", "2.5", "3.0")
//
//        val inputsss: MutableMap<String, Any> = HashMap()
//        inputsss["x"] = inputStrings
//
//        val inputFloats = inputStrings.map { it.toFloat() }.toFloatArray()
////        val inputArray = arrayOf(inputFloats)
////
//        val outputMap = mutableMapOf<String, Any>()
//        val outputArray = Array(1) { FloatArray(inputFloats.size) } // Assuming single output array
//
//        outputMap["decoded_result"] = outputArray[0]
////
//        interpreter?.runSignature(inputsss, outputMap, "decode");
        return bottleneck[0]
    }
